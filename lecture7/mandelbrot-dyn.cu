 ...
 int d, int depth) {
	...
	if(threadIdx.x == 0 && threadIdx.y == 0) {
		if(comm_dwell != DIFF_DWELL) {
			...
			// ASSIGNMENT
			int idx_x = x0 + threadIdx.x;
			int idx_y = y0 + threadIdx.y;
			if (idx_x < w && idx_y < h) {
				dwells[idx_y * w + idx_x] = comm_dwell;
			}
			...
			mandelbrot_block_k<<<grid, block>>>(dwells, w, h, cmin, cmax, x0, y0, d / SUBDIV, depth + 1);
			...
			dim3 grid(d, d);
			dim3 block(BLOCK_SIZE, BLOCK_SIZE);
			mandelbrot_pixel_k<<<grid, block>>>(dwells, w, h, cmin, cmax, x0, y0);
			cudaDeviceSynchronize();
		}
